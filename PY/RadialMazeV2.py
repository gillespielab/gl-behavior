...
if slash == '\\': # equivalent to checking if the environment is Windows (my computer) or Linux (the machines downstairs)
    ...
    filepath = f"{root}{slash}{name}_parameter-log_prelude.txt"
    filepath2 = f"{root}{slash}{name}_parameter-log_main.txt"
    
    # open a new log file
    ssi.open_log(name, f"{desktop}{slash}6-Arm_Logs")
...
class PreProcessor(Epoch):
    def __init__(self, filepath:str, maze, plot:bool = True):
        ...
        if self.parameters.max_trials > -1:
            self.X = self.parameters.max_trials + 2
        elif self.parameters.success_threshold:
            self.X = int(self.parameters.goal_blocks * outreps / self.parameters.success_threshold)
        else:
            self.X = int(self.parameters.goal_blocks * outreps) + 2
...
class RadialMaze(FileDrivenMaze):
    def __init__(self, filepath):
        ...
        # Set Epoch Transition Criteria
        self.epoch_count = 0
        self.epoch_progression = 2
    ...
    def ready(self, t):
        ...
        ssi.logger.add_line_break()
        if self.goal_blocks > 0:
            Trial.current.start = t
            self.select_goal(t)
        else:
            self.end_epoch(transition=True)
    ...
    def end_epoch(self, t = None, transition = False):
        """
        Full procedure to end the epoch with option to transition between two different parameter logs in a single run
        """
        global maze, tablepath
        ...
        # Decrement before saving/logging so the updated value is persisted
        if transition and not getattr(self, 'max_epochs_updated', False):
        ...
        if not transition:
            ...
            self.epoch_count += 1
            if self.epoch_count < self.epoch_progression:
                ssi.disp(f'Starting epoch {self.epoch_count + 1} of {self.epoch_progression}...')
                ssi.logger.add_line_break()
                ssi.close_log()
                self.pre_processor.close(t)
                ssi.open_log(name, os.path.dirname(ssi.logger.filepath))
                tablepath = ssi.logger.filepath[:-4] + '.rmTableData'
                maze = RadialMaze(filepath2)
                maze.epoch_count = self.epoch_count
                maze.start_time = time.time()
                wells.forbid_simultaneous_rewards()
                ssi.disp(f'Training Plan Index: {maze.training_plan_index}')
                ssi.logger.add_line_break()
                maze.ready(t)
            else:
                ssi.disp(f'All {self.epoch_progression} epochs complete. Session ended.')
                ssi.close_log()
                self.pre_processor.close(t)
...
def on_up(t, well):       maze.up(t, well)
def on_down(t, well):     maze.down(t, well)
def on_lockend(t=None):   maze.lockend(t)
def on_ready(t):          maze.ready(t)
def on_stop(t=None):      maze.end_epoch(t, transition=True)
...
def set_reward_error(t:int = None): maze.reward_error = True

@ssi.command('PING')
def ping(t:int = None):
    if maze.timed_out():
        maze.end_epoch(transition=True)
